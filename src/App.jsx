...
.picker-grid{display:flex;flex-wrap:wrap;gap:14px;justify-content:center;max-width:520px;}
...
.bnav-btn.act{color:var(--pul);}.bnav-ic{font-size:18px;}
...
export default function WattsHub(){
  ...
  const[choreTab,setChoreTab]=useState("assigned");
  const[choreTimeFilter,setChoreTimeFilter]=useState("week");
  const[expandedKidApprovals,setExpandedKidApprovals]=useState({});
  ...
  const getWeekStartDate=()=>{const d=new Date();const day=d.getDay();const diff=d.getDate()-day+(day===0?-6:1);d.setDate(diff);return d.toISOString().split("T")[0];};
  const getCurrentWeekKey=()=>{const d=new Date();const start=new Date(d);const day=start.getDay();const diff=start.getDate()-day+(day===0?-6:1);start.setDate(diff);return start.toISOString().split("T")[0];};
  const getCompletionsInPeriod=(period)=>{
    const endDate=today();
    let startDate;
    if(period==="week"){const d=new Date();const day=d.getDay();const diff=d.getDate()-day+(day===0?-6:1);d.setDate(diff);startDate=d.toISOString().split("T")[0];}
    else{const d=new Date();d.setDate(1);startDate=d.toISOString().split("T")[0];}
    const result={};
    for(let d=new Date(startDate);d<=new Date(endDate);d.setDate(d.getDate()+1)){
      const dk=d.toISOString().split("T")[0];
      if(comps[dk]){result[dk]=comps[dk];}
    }
    return result;
  };
  ...
  async function markHomeworkCompletion(kidId){
    const kid=kidById(kidId);if(!kid)return;
    const wk=getCurrentWeekKey();
    const streak=homeworkStreaks[kidId]||{currentStreak:0,lastMarkedDate:null,history:{}};
    const newStreak=streak.currentStreak+1;
    const rewardCents=100;const bonusRewardCents=newStreak%4===0?100:0;
    const totalRewardCents=rewardCents+bonusRewardCents;
    await fw(`wh/homeworkStreaks/${kidId}`,{...streak,currentStreak:newStreak,lastMarkedDate:wk,history:{...streak.history,[wk]:true}});
    await fm(`wh/kids/${kidId}`,{balanceCents:(kid.balanceCents||0)+totalRewardCents});
    const txId=`tx_${Date.now()}_${kidId}`;await fw(`wh/txlog/${txId}`,{id:txId,kidId,type:"homework",xp:0,cents:totalRewardCents,desc:`Homework completion week ${newStreak}${bonusRewardCents>0?" + bonus":""}`,ts:Date.now()});
  ...
  async function processPayout(kidId,amountCents){
    ...
    toast(`Payout of ${fmtDollars(amountCents)} to ${kid.name}!`,"success");
  }
  ...
  function DashboardView(){
    ...
    return(
      <div>
        ...
        {parentMode&&(
          <>
            <div style={{background:"rgba(45,212,167,0.1)",border:"1px solid rgba(45,212,167,0.3)",borderRadius:10,padding:"14px",marginBottom:14}}>
              <div style={{fontSize:12,fontWeight:700,color:"var(--te)",marginBottom:10}}>📚 This Week's Homework</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:8}}>
                {kids.map(k=>{
                  const hwData=homeworkStreaks[k.id];
                  const alreadyMarked=hwData?.history?.[weekKey];
                  return(
                    <button key={k.id} className="btn" style={{background:alreadyMarked?"var(--te)":"var(--s3)",color:alreadyMarked?"#000":"var(--tx1)",border:alreadyMarked?"1px solid var(--te)":"1px solid var(--b2)",padding:"8px 12px",borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .15s",opacity:alreadyMarked?0.8:1}}
                      onClick={()=>{if(!alreadyMarked)markHomeworkCompletion(k.id);}}>
                      {alreadyMarked?"✓ Done":"✓ Mark complete"}
                      {hwData?.currentStreak>0&&<div style={{fontSize:10,marginTop:2}}>{hwData.currentStreak} weeks</div>}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="card" style={{marginBottom:14}}>
              <div className="ch">✓ Approval Review (This Week)</div>
              {kids.map(k=>{
                const completions=getCompletionsInPeriod("week");
                const kidComps=Object.entries(completions).flatMap(([dk,items])=>
                  Object.entries(items||{}).filter(([ck,v])=>ck.endsWith(`_${k.id}`)).map(([ck,v])=>({dk,ck,v}))
                );
                const pendingComps=kidComps.filter(c=>c.v?.status==="pending");
                return(
                  <div key={k.id} style={{borderBottom:"1px solid var(--b1)",paddingBottom:10,marginBottom:10}}>
                    ...
                    {expandedKidApprovals[k.id]&&(
                      <div style={{marginTop:8,paddingLeft:8,borderLeft:"2px solid var(--b2)"}}>
                        {kidComps.length===0?<div style={{fontSize:11,color:"var(--tx3)",padding:"8px 0"}}>No completions this week</div>:(
                          kidComps.map(c=>{
                            const chore=chores.find(ch=>ch.id===c.ck.split("_")[0]);
                            return(
                              <div key={`${c.dk}_${c.ck}`} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",fontSize:11}}>
                                <span style={{width:16,height:16,borderRadius:3,background:c.v?.status==="pending"?"var(--am)":c.v?.status==="approved"?"var(--te)":"var(--s3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,flexShrink:0}}>{c.v?.status==="approved"?"✓":c.v?.status==="pending"?"⏳":""}</span>
                                <span style={{flex:1}}><strong>{chore?.title}</strong> ({c.dk})</span>
                                {c.v?.status==="pending"&&<button className="btn btn-te btn-sm" onClick={()=>approveComp(c.dk,chore?.id,k.id)}>Approve</button>}
                              </div>
                            );
                          })
                        )}
          ...
          {kids.map(k=>{
            ...
            return(
              <div key={k.id} className="kcard" onClick={()=>{setActiveKid(k.id);setView("chores");setScreen("kid");setParentMode(false);}}>
                ...
                {homeworkStreaks[k.id]&&<div className="chips">
                  {(homeworkStreaks[k.id].currentStreak||0)>0&&<span className="chip">📚 HW: {homeworkStreaks[k.id].currentStreak} weeks</span>}
                </div>}
              </div>
            );
          })}
  ...
  function PayoutModal(){
    const[mode,setMode]=useState("auto");const[customAmounts,setCustomAmounts]=useState({});
    const coinsExchangeRate=0.01;
    const handlePayout=async()=>{
      for(const kid of kids){
        let payout=0;
        if(mode==="auto"){payout=Math.floor(kid.balanceCents*coinsExchangeRate);}
        else{payout=customAmounts[kid.id]||0;}
        if(payout>0){await processPayout(kid.id,payout*100);}
      }
      setShowPayoutModal(false);
    };
    return(
      <div className="overlay" onClick={()=>setShowPayoutModal(false)}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:520}}>
        <div className="modal-h">💰 End-of-Month Payout</div>
        <div style={{fontSize:12,color:"var(--tx3)",marginBottom:16,lineHeight:1.5}}>Distribute cash to kids from their accumulated coins.</div>
        <div className="fg"><label className="fl">Payout Mode</label>
          <select className="fi" value={mode} onChange={e=>setMode(e.target.value)}>
            <option value="auto">Auto-calculate (coins × 1%)</option>
            <option value="manual">Manual amounts</option>
          </select>
        </div>
        {mode==="auto"?(
          <div style={{background:"var(--s3)",borderRadius:10,padding:12,marginBottom:14}}>
            {kids.map(k=>{const payout=Math.floor(k.balanceCents*coinsExchangeRate);return(
              <div key={k.id} style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"6px 0",borderBottom:"1px solid var(--b1)"}}>
                <span>{k.name}</span><span style={{fontWeight:700}}>{fmtDollars(payout*100)}</span>
              </div>
            );})}
          </div>
        ):(
            ...
            {kids.map(k=>(
              <div key={k.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                ...
                <div style={{display:"flex",alignItems:"center"}}>
                  ...
                  <input type="number" value={customAmounts[k.id]||""} onChange={e=>setCustomAmounts(c=>({...c,[k.id]:+e.target.value}))} style={{width:60,background:"var(--s2)",border:"1px solid var(--b2)",borderRadius:6,padding:"5px 8px",color:"var(--tx1)",fontSize:13}}/>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fax">
          <button className="btn btn-g" onClick={()=>setShowPayoutModal(false)}>Cancel</button>
          <button className="btn btn-p" onClick={handlePayout}>Process Payouts</button>
        </div>
      </div></div>
    );
  }
